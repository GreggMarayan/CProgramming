 ...
 ************************************************************/
 void initVHeap(VHeap *V)
 {
	...
	for (; ndx < VH_SIZE; ndx++)
	{
		strcpy(V->VH_node[ndx+1].elem.prodID,"    ");
		V->VH_node[ndx+1].next = ndx;
	}
	V->avail = VH_SIZE - 1;
...
void displayVHeap(VHeap V)
{
    ...
    //Write your code here 
	 for(ndx = 0; ndx < VH_SIZE; ndx++){
			printf("\n%8d", ndx);
			printf("%10s", V.VH_node[ndx].elem.prodID);
			printf("%13d", V.VH_node[ndx].next);	 
		}
...
void displaySet(cursorSet A)
{
	...
	for (int ndx = A.elemIndex; ndx != -1; ndx = A.VHptr->VH_node[ndx].next){
		printf("\n\n%-7s", A.VHptr->VH_node[ndx].elem.prodID);
		printf("%-12s", A.VHptr->VH_node[ndx].elem.prodDesc.name);
		printf("%-15d", A.VHptr->VH_node[ndx].elem.prodDesc.weight);
		printf("%-10d", ndx);
	}
...
void insertSorted(cursorSet *A, product P)
{
 	if ((A->count < VH_SIZE)){
		...
		printf("No Available space. Product ID %s could not be inserted", P.prodID);
	}
}
